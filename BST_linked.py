...
class BST:
	...
	def is_identical(self, other):
		...
		identical = self._is_identical(self._root,other._root)



		return identical

	def _is_identical(self,cur_node,other_node):
		"""
		---------------------------------------------------------
		Private helper function for is_identical
		---------------------------------------------------------
		"""

		if not cur_node and not other_node:
			return True
		if cur_node and other_node:
			if cur_node._value != other_node._value:
				return False
			if not self._is_identical(cur_node._left,other_node._left):
				return False
			if not self._is_identical(cur_node._right,other_node._right):
				return False
			return True
		return False
	...
	def max_r(self):
		...
		"""
		assert self._root is not None, "Cannot find maximum of an empty BST"
		value = self._max_r(self._root)
		return value

	def _max_r(self,cur_node):
		if not cur_node._right:
			return cur_node._value
		else:
			return self._max_r(cur_node._right)
	...
	def min_r(self):
		...
		assert self._root is not None, "Cannot find minimum of an empty BST"
		cur_node = self._root
		value = self._min_r(cur_node)
		return value

	def _min_r(self,cur_node):
		"""
		---------------------------------------------------------
		Private helper function for min_r
		---------------------------------------------------------
		"""
		if not cur_node._left:
			return cur_node._value
		else:
			return self._min_r(cur_node._left)
	...
	def _leaf_count(self,cur_node):
		"""
		Private helper function for leaf_count
		"""
		if not cur_node:
			return 0
		if (not cur_node._left and not cur_node._right):
			return 1
		else:
			return self._leaf_count(cur_node._left) + self._leaf_count(cur_node._right)
	...
	def two_child_count(self):
		...
		count = 0
		count = self._two_child_count(self._root,count)
		return count

	def _two_child_count(self,cur_node,count):
		"""
		Private helper function for two_child_count
		"""

		if not cur_node:
			return 0
		if (cur_node._left and cur_node._right):
			count += 1 + self._two_child_count(cur_node._left, count) + self._two_child_count(cur_node._right, count)
		return count
	def one_child_count(self):
		...
		"""
		count = 0
		count = self._one_child_count(self._root,count)
		return count


	def _one_child_count(self,cur_node,count):
		"""
		Private helper function for one_child_count
		"""
		...
		if not cur_node._right:
			if cur_node._left:
				count += 1+  self._one_child_count(cur_node._left,count)

		if not cur_node._left:
			if cur_node._right:
				count += 1 + self._one_child_count(cur_node._right,count)
# 		else:
# 			if cur_node._left:
# 				count += self._one_child_count(cur_node._left,count)
# 			if cur_node._right:
# 				count += self._one_child_count(cur_node._right,count)
		return count
	...
	def is_balanced(self):
		...
		left_count = 0
		right_count = 0
		left_count,right_count = self._is_balanced(self._root)
	...
	def _is_balanced(self,node):
		"""
		---------------------------------------------------------
		Private helper function for is_balanced
		---------------------------------------------------------
		"""
		...
		if not node:
			...
			left_count = self._is_balanced(node._left)
			right_count = self._is_balanced(node._right)
	...
	def is_valid(self):
		...
		return self._is_valid(self._root,INT_MIN,INT_MAX)

	def _is_valid(self,cur_node,mini,maxi):
		"""
		---------------------------------------------------------
		Private helper function for is_valid
		---------------------------------------------------------
		"""
		...
		if not cur_node:
			...
			if node._value < mini or node._value > maxi:
				return False
		return self._is_valid(cur_node._left, mini, node._value -1) and self._is_valid(cur_node._right, node._value + 1, maxi)
	...
	def inorder(self):
		...
		"""
		a = []
		a = self._inorder(self._root,a)
		return a

	def _inorder(self,node,a):
		"""
		Private helper function for inorder
		"""
		if node:
			a = self._inorder(node._left,a)
			a.append(node._value)
			a = self._inorder(node._right,a)
	...
	def preorder(self):
		...
		a = []
		a = self._preorder(self._root,a)
		return a

	def _preorder(self,node,a):
		"""
		Private helper function for preorder
		"""
		if node:
			a.append(node._value)
			a = self._preorder(node._left, a)
			a = self._preorder(node._right,a)
		return a
	...
	def postorder(self):
		...
		a = []
		a = self._postorder(self._root,a)
		return a

	def _postorder(self,node,a):
		"""
		Private helper function for postorder
		"""
		if node:
			a = self._postorder(node._left, a)
			a = self._postorder(node._right, a)
			a.append(node._value)
		return a
